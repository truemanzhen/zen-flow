---
name: zcw-build
description: "ZCW Phase 3: Plan and Build. Invoke with /zcw-build. Create plans and select execution method (subagent or direct) for implementation."
---

# ZCW Phase 3: Plan and Build (Build)
...
**Idempotency**: All build phase operations can be safely re-executed. Read `.zcw.yaml` `phase` field to confirm still in build, read plan header `base-ref`, then use `grep -n '\- \[ \]' tasks.md | head -1` to find the first unchecked task. Already-committed tasks must not be re-committed.
...
3. Read `specs/<name>/tasks.md` (task boundaries)
...
| B | Pause to switch model | Record `build_pause: plan-ready`, stop this `/zcw-build` invocation, and allow the user to resume later from `/zcw` or `/zcw-build` |

This is a user decision point. **Must follow the `zcw/reference/decision-point.md` protocol to pause and wait for the user to explicitly choose**. Must not auto-continue and must not write the pause into `build_mode`.
...
- If the user chooses `executing-plans`: run `"$ZCW_BASH" "$ZCW_STATE" set <name> subagent_dispatch null`, then run `"$ZCW_BASH" "$ZCW_STATE" set <name> build_mode executing-plans`
- If the user chooses `subagent-driven-development`: first confirm the current platform has real background subagent / Task / multi-agent dispatch capability; after confirming, run `"$ZCW_BASH" "$ZCW_STATE" set <name> subagent_dispatch confirmed`, then run `"$ZCW_BASH" "$ZCW_STATE" set <name> build_mode subagent-driven-development`
...
- **branch**: Recommend a branch name based on the workflow type and current date, then let the user confirm or input a custom name. This is a user decision point — **must use the current platform's available user input/confirmation mechanism to pause and wait for the user to explicitly confirm or override the branch name**. Must not skip this step and create the branch directly.
  ...
  - Read the `workflow` field from `.zcw.yaml` to determine the prefix
...
- `build_mode: subagent-driven-development`: The main session only coordinates and must not write implementation code directly. **Immediately execute:** Use the Skill tool to load the Superpowers `subagent-driven-development` skill. After the skill loads, read `zcw/reference/subagent-dispatch.md` for ZCW-specific extensions (real background dispatch, task isolation, checkoff verification, TDD constraints, continuous execution, context recovery) and apply them alongside the skill's workflow. If they conflict, the more specific ZCW extensions take precedence.
- If the current platform has no real background agent dispatch capability, must pause and wait for the user to choose main window execution instead. After the user chooses, must run `"$ZCW_BASH" "$ZCW_STATE" set <name> build_mode executing-plans`, then follow the `build_mode: executing-plans` branch to load the Superpowers `executing-plans` skill. Must not continue executing tasks before the user explicitly chooses.
...
- `build_mode: subagent-driven-development`: The main session does not load the TDD skill. TDD constraints and evidence thresholds are defined in `zcw/reference/subagent-dispatch.md`; every background implementer and fix agent must use the Skill tool to load the Superpowers `test-driven-development` skill and follow the ZCW-injected TDD hard constraint.
...
|------|-------------------|----------|
| Small | Missing acceptance scenarios, edge cases | Directly edit delta spec + plan.md, append tasks.md tasks |
...
| Large | Brand-new capability requirements | **Must use the current platform's available user input/confirmation mechanism to pause and wait for the user to explicitly confirm the split**; after user confirms, create independent change through `/zcw-open` |

**50% Threshold Determination**: Using initial task count in tasks.md as baseline, if new tasks exceed half of that total, it's considered outside original plan scope, **must follow the `zcw/reference/decision-point.md` protocol to pause and wait for the user to decide whether to split into a new change**.

When creating an independent change, must invoke `/zcw-open`, not `/opsx:new` directly. `/zcw-open` creates both Spec Kit artifacts and `.zcw.yaml`, preventing the new change from leaving the ZCW state machine.

**User choices must include**:
- "Split into new change" — create independent change via `/zcw-open`
...
- **Context compression recovery**: Follow `zcw/reference/context-recovery.md` with phase set to `build`.
- **User manual-change resume**: handle uncommitted changes through `zcw/reference/dirty-worktree.md`. That protocol defines checks, attribution, and prohibitions. Build-specific handling:
...
Configuration can live in the change `.zcw.yaml`, or in repo-root `.zcw.yaml` / `zcw.yaml` / `.zcw.yml` / `zcw.yml`.
