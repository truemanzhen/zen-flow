---
name: zcw-hotfix
description: "ZCW preset path: Bug fix / hotfix. Skip brainstorming, directly open → build → verify → archive. Applicable for behavior fixes, scenarios not involving new capability design."
---

# ZCW Preset Path: Hotfix
...
**Not applicable**: If fix process discovers need for architecture adjustments, should upgrade to full `/zcw` workflow.
...
Reuse ZCW open capability to create change, but use hotfix defaults: do not execute `speckit-explore` long exploration, directly enter streamlined change creation.
...
After the skill loads, follow its guidance to create streamlined artifacts:
  - `spec.md` — problem description + root cause analysis + fix goal (no solution comparison needed)
  - `plan.md` — fix solution (one is enough, no multi-solution comparison needed)
...
- Create delta spec in `specs/<name>/specs/<capability>/spec.md`
...
1. Read bug description and root cause in spec.md
...
Reuse `/zcw-verify`, with zcw-verify's scale assessment deciding lightweight or full verification.
...
If there is delta spec, sync to main spec according to zcw-archive rules, and handle associated Design Doc and Plan archiving annotations.
...
<IMPORTANT>
Hotfix workflow is **one-time continuous execution**. After invoking `/zcw-hotfix`, agent must automatically advance through hotfix steps, without pausing to wait for user input mid-way.
...
2. workspace isolation and execution-method selection when tasks exceed 3 and transfer to `/zcw-build`
3. verify phase (zcw-verify) verification-failure and branch-handling decisions
4. Final archive confirmation (before zcw-archive runs the archive script)

Execution order: quick open → direct build → root cause check → verification → archive → complete

After each step completes, immediately enter next step. Within each phase, must still call corresponding ZCW/Spec Kit/Superpowers skill according to above requirements; if the called skill has its own user decision points, follow that skill's rules.
</IMPORTANT>
...
Then on current change basis, supplement Design Doc: **Immediately use the Skill tool to load the `zcw-design` skill**, proceed normally with full workflow. If user does not confirm upgrade, stop hotfix and report that current change has exceeded hotfix scope.
...
- `NEXT: auto` → invoke the skill pointed to by `SKILL` to continue hotfix workflow (`phase: build` returns `zcw-hotfix`, `verify` returns `zcw-verify`, `archive` returns `zcw-archive`)
