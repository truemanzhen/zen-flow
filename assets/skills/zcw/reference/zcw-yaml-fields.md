...
Canonical path: `zcw/reference/zcw-yaml-fields.md`

This file is the field reference for the `.zcw.yaml` state file. Consult on demand; not loaded inline with skills.
...
workflow: full
phase: build
design_doc: docs/superpowers/specs/YYYY-MM-DD-topic-plan.md
...
| `isolation` | `branch` or `worktree`, workspace isolation mode. Full init may be `null` but only until `/zcw-build` Step 3; hotfix/tweak default to `branch` |
| `verify_mode` | `light` or `full`; may be empty |
| `auto_transition` | `true` or `false`. Only controls whether to automatically invoke the next skill after phase guard advances phase; `false` outputs `manual` from `zcw-state next`, pausing next-skill invocation but not blocking phase field updates |
...
- These constraints exist in both `zcw-guard.sh build --apply` and `zcw-state.sh transition <name> build-complete`
