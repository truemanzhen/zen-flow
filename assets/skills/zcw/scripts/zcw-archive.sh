#!/bin/bash
# ZCW Archive — automates the archive phase in one command
# Usage: zcw-archive.sh <change-name> [--dry-run]
...
ARCHIVE_DIR="specs/archive/${ARCHIVE_NAME}"
...
echo "=== ZCW Archive: $CHANGE ===" >&2

# --- Step 1: Read .zcw.yaml, extract paths ---

yaml_field() {
  local field="$1"
  if [ -f "$STATE_SH" ]; then
    "$ZCW_BASH" "$STATE_SH" get "$CHANGE" "$field" 2>/dev/null
  else
    if [ -f "$YAML" ]; then
      local value
...
if [ "$VERIFY_VAL" != "pass" ]; then
  red "FATAL: verify_result is '$VERIFY_VAL', expected 'pass'. Run zcw-verify first."
  exit 1
fi
...
# --- Step 5: Archive the Spec Kit change directory ---

verify_main_specs_clean() {
  if [ ! -d "specs/main" ]; then
    return 0
  fi

  local found=0
  local matches
  for spec_file in specs/main/*/spec.md; do
...
resolve_archive_dir() {
  ...
  local found
  found=$(find "specs/archive" -maxdepth 1 -mindepth 1 -type d -name "*-$CHANGE" 2>/dev/null | head -1 || true)
  if [ -n "$found" ]; then
    ARCHIVE_DIR="$found"
...
if [ "$DRY_RUN" -eq 1 ]; then
  step_dry_run "Would archive Spec Kit change: $CHANGE_DIR -> $ARCHIVE_DIR"
else
  mkdir -p "specs/archive"
  mv "$CHANGE_DIR" "$ARCHIVE_DIR"
  if ! resolve_archive_dir; then
    step_fail "Spec Kit archive output not found"
    exit 1
  else
    step_ok "Spec Kit archive completed: $ARCHIVE_DIR"
  fi
...
# --- Step 7: Mark archived via zcw-state transition ---

ARCHIVE_YAML="$ARCHIVE_DIR/.zcw.yaml"

if [ "$DRY_RUN" -eq 1 ]; then
  step_dry_run "Would set archived: true in $ARCHIVE_YAML"
else
  if [ -f "$ARCHIVE_YAML" ]; then
    "$ZCW_BASH" "$STATE_SH" transition "$ARCHIVE_NAME" archived >/dev/null
    step_ok "archived: true"
  else
    step_fail "archived: true (.zcw.yaml not found after move)"
  fi
fi
