#!/bin/bash
# zcw-hook-guard.sh — PreToolUse hook for ZCW phase enforcement
#
# Blocks file writes (Write/Edit) when the active ZCW change is in
...
if [ -z "$TARGET" ]; then
  echo "[ZCW-HOOK] allowed: no file path in tool input" >&2
  exit 0
fi
...
# ── Helpers to read .zcw.yaml fields ───────────────────────────
...
# ── Determine the governing ZCW change + phase ─────────────────
#
# A write targeting a specific change directory (specs/<name>/...)
...
PHASE=""
# Path to the .zcw.yaml that governs this write (used for deeper invariant checks)
GOV_YAML=""
  ...
  specs/*/*)
    ...
    if [ -n "$_own_change" ] && [ "$_own_change" != "archive" ]; then
      ...
      if [ -f "$_own_yaml" ]; then
        ...
        # (artifacts are created before .zcw.yaml during /zcw-open).
...
if [ -z "$PHASE" ]; then
  ...
  if [ -d "specs" ]; then
    for dir in specs/*/; do
      ...
      if [ -f "${dir}.zcw.yaml" ]; then
        # Skip changes already marked as archived
        if [ "$(is_archived "${dir}.zcw.yaml")" = "true" ]; then
          continue
        fi
        YAML_FILE="${dir}.zcw.yaml"
        break
      fi
    done
  fi
  ...
  if [ -z "$YAML_FILE" ]; then
    echo "[ZCW-HOOK] allowed: no active zcw change" >&2
    exit 0
  fi
...
if [ -z "$PHASE" ]; then
  echo "[ZCW-HOOK] allowed: no phase in .zcw.yaml" >&2
  exit 0
fi
...
case "$RELPATH" in
  specs/*)
    # Spec Kit artifacts — phase-aware sub-check
    case "$PHASE" in
      open)
        # open: allow proposal, design, tasks, yaml, handoff, specs
        case "$RELPATH" in
          */spec.md|*/plan.md|*/tasks.md|*/.zcw.yaml|*/.zcw/*|*/specs/*)
            echo "[ZCW-HOOK] allowed: $RELPATH (phase: open, speckit artifacts)" >&2
            exit 0
            ;;
        esac
        ;;
      design)
        # design: allow handoff, delta spec (Spec Patch), proposal/design/tasks (minor refinements), .zcw.yaml
        case "$RELPATH" in
          */spec.md|*/plan.md|*/tasks.md|*/.zcw/*|*/specs/*|*/.zcw.yaml)
            echo "[ZCW-HOOK] allowed: $RELPATH (phase: design, handoff/spec)" >&2
            exit 0
            ;;
        esac
        ;;
      build)
        # build: allow delta spec (incremental update), tasks, .zcw.yaml
        case "$RELPATH" in
          */specs/*|*/tasks.md|*/.zcw.yaml)
            echo "[ZCW-HOOK] allowed: $RELPATH (phase: build, spec/tasks)" >&2
            exit 0
            ;;
        esac
        ;;
      verify)
        # verify: allow tasks (post-check), .zcw.yaml
        case "$RELPATH" in
          */tasks.md|*/.zcw.yaml)
            echo "[ZCW-HOOK] allowed: $RELPATH (phase: verify, tasks/state)" >&2
            exit 0
            ;;
        esac
        ;;
      archive)
        # archive: allow .zcw.yaml state updates only
        case "$RELPATH" in
          */.zcw.yaml)
            echo "[ZCW-HOOK] allowed: $RELPATH (phase: archive, state)" >&2
            exit 0
            ;;
        esac
        ;;
  ...
  docs/superpowers/*)
    ...
    case "$PHASE" in
      design)
        echo "[ZCW-HOOK] allowed: $RELPATH (phase: design, superpowers)" >&2
        exit 0
        ;;
      build)
        echo "[ZCW-HOOK] allowed: $RELPATH (phase: build, superpowers)" >&2
        exit 0
        ;;
      verify)
        echo "[ZCW-HOOK] allowed: $RELPATH (phase: verify, superpowers)" >&2
        exit 0
        ;;
    esac
  ...
  .zcw/*|*/.zcw/*)
    # ZCW config
    echo "[ZCW-HOOK] allowed: $RELPATH (whitelist: zcw config)" >&2
    exit 0
    ;;
  .specify/*|*/.specify/*)
    # Spec Kit config
    echo "[ZCW-HOOK] allowed: $RELPATH (whitelist: spec-kit config)" >&2
    exit 0
    ;;
  .claude/*)
    # Claude settings/rules
    echo "[ZCW-HOOK] allowed: $RELPATH (whitelist: claude config)" >&2
    exit 0
    ;;
  CLAUDE.md|CHANGELOG.md|README.md|*.md)
    ...
    case "$RELPATH" in
      ...
      *)
        echo "[ZCW-HOOK] allowed: $RELPATH (whitelist: root markdown)" >&2
        exit 0
        ;;
    esac
    ;;
  .zcw.yaml|zcw.yaml|.zcw.yml|zcw.yml)
    # Project-level zcw config
    echo "[ZCW-HOOK] allowed: $RELPATH (whitelist: zcw config)" >&2
    exit 0
    ;;
esac
  ...
  build|verify)
    ...
    if [ -n "$GOV_YAML" ]; then
      ...
      if [ "$_wf" = "full" ] && { [ -z "$_dd" ] || [ "$_dd" = "null" ]; }; then
        echo "" >&2
        echo "╔══════════════════════════════════════════╗" >&2
        echo "║     ZCW PHASE GUARD — WRITE BLOCKED    ║" >&2
        echo "╚══════════════════════════════════════════╝" >&2
        echo "" >&2
        ...
        echo "  ✅ Correct flow: create the Design Doc in design phase, then run zcw-guard design --apply" >&2
        echo "  💡 Run /zcw-design to fill the missing design; for repair, set design_doc with zcw-state" >&2
        echo "" >&2
        exit 2
      fi
    fi
    # Code writes allowed in build and verify
    echo "[ZCW-HOOK] allowed: $RELPATH (phase: $PHASE)" >&2
    exit 0
    ;;
  open|design|archive)
    echo "" >&2
    ...
    case "$PHASE" in
      ...
      design)
        ...
        echo "  💡 After the Design Doc is ready, run zcw-guard design --apply to enter build" >&2
        ;;
      archive)
