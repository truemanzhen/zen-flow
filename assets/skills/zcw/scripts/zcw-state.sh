#!/bin/bash
# ZCW State — unified interface for .zcw.yaml state management
# Usage: zcw-state.sh <subcommand> <change-name> [args...]
#
# Subcommands:
#   init <change-name> <workflow>  — Initialize .zcw.yaml with workflow defaults
#   get <change-name> <field>       — Read a field value from .zcw.yaml
...
change_dir_for() {
  local change_name="$1"
  if [ -d "specs/$change_name" ]; then
    echo "specs/$change_name"
  elif [ -d "specs/archive/$change_name" ]; then
    echo "specs/archive/$change_name"
  else
    echo "specs/$change_name"
  fi
}
...
yaml_file_for() {
  ...
  echo "$change_dir/.zcw.yaml"
}

project_context_compression() {
  local value="off"
  local source="default"
  if [ -n "${ZCW_CONTEXT_COMPRESSION:-}" ]; then
    value="$ZCW_CONTEXT_COMPRESSION"
    source="ZCW_CONTEXT_COMPRESSION"
  elif [ -f ".zcw/config.yaml" ]; then
    value=$(yaml_field "context_compression" ".zcw/config.yaml")
    value="${value:-off}"
    source=".zcw/config.yaml"
  fi
...
project_auto_transition_default() {
  local value="true"
  local source="default"
  if [ -n "${ZCW_AUTO_TRANSITION:-}" ]; then
    value="$ZCW_AUTO_TRANSITION"
    source="ZCW_AUTO_TRANSITION"
  elif [ -f ".zcw/config.yaml" ]; then
    local raw
    raw=$(yaml_field "auto_transition" ".zcw/config.yaml" 2>/dev/null || true)
    if [ -n "$raw" ]; then
      value="$raw"
      source=".zcw/config.yaml"
    fi
...
project_review_mode_default() {
  local value="null"
  local source="default"
  if [ -n "${ZCW_REVIEW_MODE:-}" ]; then
    value="$ZCW_REVIEW_MODE"
    source="ZCW_REVIEW_MODE"
  elif [ -f ".zcw/config.yaml" ]; then
    local raw
    raw=$(yaml_field "review_mode" ".zcw/config.yaml" 2>/dev/null || true)
    if [ -n "$raw" ]; then
      value="$raw"
      source=".zcw/config.yaml"
    fi
...
cmd_init() {
  ...
  # Check if .zcw.yaml already exists
  if [ -f "$yaml_file" ]; then
    red "ERROR: .zcw.yaml already exists at $yaml_file"
    exit 1
  fi
...
cmd_get() {
  ...
  # Check if .zcw.yaml exists
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .zcw.yaml not found at $yaml_file"
    exit 1
  fi
...
cmd_set() {
  ...
  case "$field" in
    phase)
      ...
      if [ "${_ZCW_IN_TRANSITION:-}" != "1" ] && [ "${ZCW_FORCE_PHASE:-}" != "1" ]; then
        red "ERROR: Setting 'phase' directly is not allowed; it bypasses state machine evidence checks." >&2
        red "  Use: zcw-state.sh transition <change-name> <event>" >&2
        red "  Repair-only escape hatch: ZCW_FORCE_PHASE=1 zcw-state.sh set <change-name> phase <value>" >&2
        exit 1
      fi
...
require_open_artifacts() {
  ...
  if [ "$workflow" = "full" ] && [ ! -s "$change_dir/plan.md" ]; then
    red "ERROR: Cannot transition '$change_name': plan.md must exist and be non-empty before leaving open" >&2
    exit 1
  fi
}
...
cmd_check() {
  ...
  local change_dir="specs/$change_name"
  local yaml_file="$change_dir/.zcw.yaml"
  local proposal_file="$change_dir/spec.md"
  local design_file="$change_dir/plan.md"
  local tasks_file="$change_dir/tasks.md"

  echo "=== Entry Check: zcw-${phase} ==="

  # .zcw.yaml must exist for all phases (state machine core)
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .zcw.yaml not found at $yaml_file"
    exit 1
  fi
  ...
  case "$phase" in
    ...
    design)
      ...
      check_nonempty "spec.md" "$proposal_file"
      check_nonempty "plan.md" "$design_file"
  ...
  case "$phase" in
    open)
      ...
      if [ "$artifacts_done" -eq 3 ]; then
        echo "Recovery action: All artifacts complete. Run /zcw-open user confirmation, then guard to transition."
      elif [ "$artifacts_done" -eq 0 ]; then
        echo "Recovery action: No artifacts created yet. Start from /zcw-open Step 1 (explore and clarify)."
      else
        echo "Recovery action: Some artifacts incomplete. Resume /zcw-open from the first missing artifact."
      fi
      ;;
    design)
      echo "  Artifacts:"
    ...
    build)
      ...
      if [ "$build_pause" = "plan-ready" ] && [ -n "$plan" ] && [ "$plan" != "null" ] && [ -f "$plan" ] && { [ "$isolation" = "null" ] || [ -z "$isolation" ] || [ "$build_mode" = "null" ] || [ -z "$build_mode" ]; }; then
        ...
        if [ "$build_mode" = "subagent-driven-development" ]; then
          if [ "$subagent_dispatch" = "confirmed" ]; then
            echo "Recovery action: Read tasks.md and the Superpowers plan (which may include additions beyond Spec Kit), then inspect the first unchecked task against recent git history/diff. If implemented, check it off; otherwise dispatch a real background subagent. Do not execute the pending task directly in the main window."
          else
    ...
    verify)
      ...
      if [ "$verify_result" = "pass" ] && [ "$branch_status" = "handled" ]; then
        ...
        echo "Recovery action: Verification failed and rolled back to build. Resume from /zcw-build."
      else
    ...
    archive)
      ...
      echo ""
      echo "Recovery action: Run /zcw-archive to complete archiving."
      ;;
    *)
...
cmd_scale() {
  ...
  # 3. Changed files: prefer plan base-ref, then .zcw.yaml base_ref, fall back to worktree diff
  ...
  if git rev-parse --git-dir > /dev/null 2>&1; then
    ...
    # Fallback to base_ref stored in .zcw.yaml (set during init)
  ...
  # Update verify_mode in .zcw.yaml
...
cmd_next() {
  ...
  local yaml_file="$change_dir/.zcw.yaml"
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .zcw.yaml not found at $yaml_file" >&2
    exit 1
  fi
  ...
  case "$phase" in
    ...
    build)
      case "$workflow" in
        hotfix) skill="zcw-hotfix" ;;
        tweak)  skill="zcw-tweak" ;;
        *)      skill="zcw-build" ;;
      esac
      ;;
    verify)
...
case "$SUBCOMMAND" in
  init)
    if [ $# -lt 2 ]; then
      red "Usage: zcw-state.sh init <change-name> <workflow>" >&2
  ...
  get)
    if [ $# -lt 2 ]; then
      red "Usage: zcw-state.sh get <change-name> <field>" >&2
      exit 1
    fi
    cmd_get "$@"
    ;;
  set)
    if [ $# -lt 3 ]; then
      red "Usage: zcw-state.sh set <change-name> <field> <value>" >&2
      exit 1
    fi
    cmd_set "$@"
    ;;
  transition)
    if [ $# -lt 2 ]; then
      red "Usage: zcw-state.sh transition <change-name> <event>" >&2
  ...
  check)
    if [ $# -lt 2 ]; then
      red "Usage: zcw-state.sh check <change-name> <phase> [--recover]" >&2
  ...
  scale)
    if [ $# -lt 1 ]; then
      red "Usage: zcw-state.sh scale <change-name>" >&2
      exit 1
    fi
    cmd_scale "$@"
    ;;
  task-checkoff)
    if [ $# -lt 2 ]; then
      red "Usage: zcw-state.sh task-checkoff <file> <task-text>" >&2
      exit 1
    fi
    cmd_task_checkoff "$@"
    ;;
  next)
    if [ $# -lt 1 ]; then
      red "Usage: zcw-state.sh next <change-name>" >&2
      exit 1
    fi
    cmd_next "$@"
    ;;
  *)
    ...
    echo "" >&2
    echo "Usage: zcw-state.sh <subcommand> <change-name> [args...]" >&2
    echo "" >&2
    echo "Subcommands:" >&2
    echo "  init <change-name> <workflow>  — Initialize .zcw.yaml with workflow defaults" >&2
    echo "  get <change-name> <field>       — Read a field value from .zcw.yaml" >&2
    echo "  set <change-name> <field> <val> — Update a field value in .zcw.yaml" >&2
