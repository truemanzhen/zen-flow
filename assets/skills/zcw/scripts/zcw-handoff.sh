#!/bin/bash
# ZCW Handoff — creates machine-owned context packages between phases
# Usage: zcw-handoff.sh <change-name> design --write [--full]
#        zcw-handoff.sh <change-name> --hash-only
...
write_markdown_context() {
  local output="$1"
  {
    ...
    echo ""
    echo "Generated-by: zcw-handoff.sh"
    echo ""
    echo "Spec Kit remains the canonical capability spec. This handoff is a deterministic, source-traceable context pack, not an agent-authored summary."
    echo ""
...
write_json_context() {
  local output="$1"
  {
    ...
    echo "  \"phase\": \"design\","
    echo "  \"mode\": \"$HANDOFF_MODE\","
    echo "  \"canonical_spec\": \"speckit\","
    echo "  \"generated_by\": \"zcw-handoff.sh\","
...
write_spec_markdown_context() {
  local output="$1"
  {
    ...
    echo "Generated-by: zcw-handoff.sh"
    echo ""
    echo "Spec Kit remains the canonical capability spec. This beta context pack verbatim-projects spec files and references supporting artifacts by hash, not an agent-authored summary."
    echo ""
...
if [ "$PHASE" != "design" ] || [ "$MODE" != "--write" ]; then
  red "Usage: zcw-handoff.sh <change-name> design --write [--full]"
  exit 1
fi
case "$FULL_FLAG" in
...
if [ ! -f "$YAML" ]; then
  red "ERROR: .zcw.yaml not found at $YAML"
  exit 1
fi
...
for required in spec.md plan.md tasks.md; do
  if [ ! -s "$CHANGE_DIR/$required" ]; then
    red "ERROR: required Spec Kit artifact missing or empty: $CHANGE_DIR/$required"
    exit 1
  fi
done

HANDOFF_DIR="$CHANGE_DIR/.zcw/handoff"
...
if [ -x "$STATE_SH" ] || [ -f "$STATE_SH" ]; then
  "$ZCW_BASH" "$STATE_SH" set "$CHANGE" handoff_context "$CONTEXT_JSON" >/dev/null
  "$ZCW_BASH" "$STATE_SH" set "$CHANGE" handoff_hash "$CONTEXT_HASH" >/dev/null
else
  red "ERROR: zcw-state.sh not found; cannot record handoff fields"
  exit 1
fi
