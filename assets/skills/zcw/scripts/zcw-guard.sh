#!/bin/bash
# ZCW Phase Guard — validates exit conditions before phase transitions
# Usage: zcw-guard.sh <change-name> <current-phase> [--apply]
...
if [ "${ZCW_GUARD_SOURCE_ONLY:-0}" = "1" ]; then
  CHANGE="${CHANGE:-}"
  PHASE="${PHASE:-}"
  APPLY="${APPLY:-0}"
  ...
  CHANGE_DIR="specs/$CHANGE"
  if [ "$PHASE" = "archive" ] && [ ! -d "$CHANGE_DIR" ] && [ -d "specs/archive/$CHANGE" ]; then
    CHANGE_DIR="specs/archive/$CHANGE"
  fi
fi
...
plan_tasks_all_done() {
  ...
  if [ ! -f "$plan" ]; then
    ...
    echo "Next: restore the Superpowers plan file or update .zcw.yaml plan before leaving build." >&2
    return 1
  fi
...
yaml_field_value() {
  local field="$1"
  local yaml="$CHANGE_DIR/.zcw.yaml"
  if [ -f "$yaml" ]; then
    local value
...
project_config_value() {
  ...
  for config in ".zcw.yaml" "zcw.yaml" ".zcw.yml" "zcw.yml"; do
    if [ -f "$config" ]; then
...
preflight() {
  ...
  if [ ! -f "$CHANGE_DIR/.zcw.yaml" ]; then
    red "FATAL: .zcw.yaml not found in $CHANGE_DIR"
    exit 1
  fi

  # Schema validation
  local validate_script
  validate_script="$SCRIPT_DIR/zcw-yaml-validate.sh"
  if [ -f "$validate_script" ]; then
    if ! "$ZCW_BASH" "$validate_script" "$CHANGE" 2>/dev/null; then
      "$ZCW_BASH" "$validate_script" "$CHANGE" || true
      red "FATAL: .zcw.yaml schema validation failed"
      exit 1
    fi
  fi
}
...
isolation_selected() {
  ...
  case "$isolation" in
    ...
    *)
      ...
      echo "  \"\$ZCW_BASH\" \"\$ZCW_STATE\" set $CHANGE isolation <branch|worktree>" >&2
      return 1
      ;;
  esac
}
...
build_mode_selected() {
  ...
  case "$build_mode" in
    ...
    *)
      ...
      echo "  \"\$ZCW_BASH\" \"\$ZCW_STATE\" set $CHANGE build_mode <subagent-driven-development|executing-plans>" >&2
      return 1
      ;;
  esac
}
...
subagent_dispatch_confirmed() {
  ...
  echo "  \"\$ZCW_BASH\" \"\$ZCW_STATE\" set $CHANGE subagent_dispatch confirmed" >&2
  echo "Or ask the user to switch to executing-plans and run:" >&2
  echo "  \"\$ZCW_BASH\" \"\$ZCW_STATE\" set $CHANGE build_mode executing-plans" >&2
  return 1
}
...
tdd_mode_selected() {
  ...
  case "$tdd_mode" in
    ...
    *)
      ...
      echo "  \"\$ZCW_BASH\" \"\$ZCW_STATE\" set $CHANGE tdd_mode <tdd|direct>" >&2
      return 1
      ;;
  esac
}
...
review_mode_selected() {
  ...
  case "$review_mode" in
    ...
    *)
      ...
      echo "  \"\$ZCW_BASH\" \"\$ZCW_STATE\" set $CHANGE review_mode <off|standard|thorough>" >&2
      return 1
      ;;
  esac
}
...
design_handoff_context_valid() {
  ...
  if [ -z "$context" ] || [ "$context" = "null" ]; then
    echo "handoff_context is missing from .zcw.yaml" >&2
    echo "Next: run \"\$ZCW_BASH\" \"\$ZCW_HANDOFF\" $CHANGE design --write before invoking Superpowers." >&2
    return 1
  fi
  if [ ! -s "$context" ]; then
    echo "handoff_context does not point to a non-empty file: $context" >&2
    echo "Next: regenerate the design handoff with zcw-handoff.sh." >&2
    return 1
  fi
  ...
  if [ "$actual_hash" != "$recorded_hash" ]; then
    echo "Spec Kit artifacts changed after handoff was generated." >&2
    echo "Expected handoff_hash: $recorded_hash" >&2
    echo "Actual handoff_hash:   $actual_hash" >&2
    echo "Next: rerun zcw-handoff.sh so Superpowers receives the current Spec Kit context." >&2
    return 1
  fi
...
design_handoff_markdown_traceable() {
  ...
  grep -q '^Generated-by: zcw-handoff\.sh$' "$markdown" || {
...
design_doc_links_current_change() {
  ...
  design_doc_frontmatter_has "$design_doc" "zcw_change" "$CHANGE"
}
...
design_doc_declares_canonical_spec() {
  ...
  design_doc=$(yaml_field_value "design_doc" 2>/dev/null || true)
  [ -n "$design_doc" ] && [ "$design_doc" != "null" ] && [ -s "$design_doc" ] &&
    design_doc_frontmatter_has "$design_doc" "canonical_spec" "speckit"
}
...
guard_open() {
  ...
  check "spec.md exists and non-empty" file_nonempty "$CHANGE_DIR/spec.md"
  if [ "$workflow" = "full" ]; then
    check "plan.md exists and non-empty" file_nonempty "$CHANGE_DIR/plan.md"
  fi
...
guard_design() {
  ...
  check "spec.md exists" file_nonempty "$CHANGE_DIR/spec.md"
  check "plan.md exists" file_nonempty "$CHANGE_DIR/plan.md"
  ...
  if [ -n "$design_doc" ] && [ "$design_doc" != "null" ]; then
    ...
    check "Design Doc declares Spec Kit as canonical spec" design_doc_declares_canonical_spec
  elif [ "$workflow" != "full" ]; then
    warn "  [WARN] No design_doc recorded in .zcw.yaml (optional for hotfix/tweak)"
  fi
}
...
design_doc_recorded() {
  ...
  echo "Next: create the Design Doc and run: \"\$ZCW_BASH\" \"\$ZCW_STATE\" set $CHANGE design_doc <path>" >&2
  return 1
}
...
apply_state_update() {
  local state_sh="$SCRIPT_DIR/zcw-state.sh"
  local p="$1"

  if [ -f "$state_sh" ]; then
    case "$p" in
      open)   "$ZCW_BASH" "$state_sh" transition "$CHANGE" open-complete ;;
      design) "$ZCW_BASH" "$state_sh" transition "$CHANGE" design-complete ;;
      build)  "$ZCW_BASH" "$state_sh" transition "$CHANGE" build-complete ;;
      verify) "$ZCW_BASH" "$state_sh" transition "$CHANGE" verify-pass ;;
    esac
  else
    red "FATAL: zcw-state.sh not found; cannot apply state transition"
    exit 1
  fi
}
...
if [ "${ZCW_GUARD_SOURCE_ONLY:-0}" = "1" ]; then
  ...
  red "ERROR: ZCW_GUARD_SOURCE_ONLY=1 is only for sourcing, not direct execution" >&2
  ...
  if [ "$APPLY" -eq 1 ]; then
    ...
    case "$PHASE" in
      open)
        ...
        green "  [APPLY] .zcw.yaml updated: phase=$new_phase"
        ;;
      design) green "  [APPLY] .zcw.yaml updated: phase=build" ;;
      build)  green "  [APPLY] .zcw.yaml updated: phase=verify, verify_result=pending" ;;
      verify) green "  [APPLY] .zcw.yaml updated: phase=archive, verify_result=pass" ;;
    esac
  fi
  exit 0
fi
