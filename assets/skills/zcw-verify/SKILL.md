---
name: zcw-verify
description: "ZCW Phase 4: Verify and Close. Invoke with /zcw-verify. Verify implementation matches design, handle development branch."
---

# ZCW Phase 4: Verify and Close (Verify)
...
. "$ZCW_ENV"
"$ZCW_BASH" "$ZCW_STATE" check <change-name> verify
```
...
```bash
"$ZCW_BASH" "$ZCW_STATE" scale <change-name>
```
...
Before verification begins, handle uncommitted changes through `zcw/reference/dirty-worktree.md` protocol. Verify phase special handling:
...
```bash
PLAN=$("$ZCW_BASH" "$ZCW_STATE" get <change-name> plan)
...
```bash
"$ZCW_BASH" "$ZCW_STATE" set <change-name> verify_mode full
```

**Override mechanism**: If the agent or user believes the automated assessment is inappropriate, override at any time with `"$ZCW_BASH" "$ZCW_STATE" set <change-name> verify_mode <light|full>`.

### 1b. Verification Failure Decision (Blocking Point)

When verification does not pass, **must follow the `zcw/reference/decision-point.md` protocol to pause and wait for the user to decide whether to fix or accept the deviation**. Must not automatically run `"$ZCW_BASH" "$ZCW_STATE" transition <change-name> verify-fail`, nor automatically invoke `/zcw-build`.
...
- **Fix all**: Run `"$ZCW_BASH" "$ZCW_STATE" transition <change-name> verify-fail`, then invoke `/zcw-build` to fix
...
When verification needs to read Spec Kit artifacts, first check whether they have changed since the design phase:

```bash
RECORDED_HASH=$("$ZCW_BASH" "$ZCW_STATE" get <change-name> handoff_hash)
CURRENT_HASH=$("$ZCW_BASH" "$ZCW_HANDOFF" <change-name> --hash-only 2>/dev/null || echo "")
```

- If `RECORDED_HASH` = `CURRENT_HASH` and both are non-empty and neither is `null`: Spec Kit artifacts are unchanged. **tasks.md does not need to be re-read in full** (use `grep -c '\- \[ \]' tasks.md` to confirm completion count). spec.md, plan.md, and delta specs must still be read for comparison checks.
...
This optimization only skips re-reading tasks.md in full. spec.md and plan.md contain the full context needed for verification checks and must not be skipped due to hash match.
...
**When not passing**: Report failures, enter Step 1b verification failure decision blocking point. Only after user confirms fix, execute the following command to record failure and roll back to build phase, then invoke `/zcw-build` to fix:
...
**Immediately execute:** Use the Skill tool to load the `speckit-verify-change` skill. Skipping this step is prohibited.

After the skill loads, follow its guidance to verify. Check items:
1. All tasks.md tasks completed (`[x]`)
2. Implementation matches `specs/<name>/plan.md` high-level design decisions
...
When verification does not pass: report missing items, enter Step 1b verification failure decision blocking point. Only after user confirms fix, execute the following command to record failure and roll back to build phase, then invoke `/zcw-build` to supplement:
...
- If check item 6 finds contradictions (delta spec has content but design doc does not reflect it), **must use the current platform's available user input/confirmation mechanism as a single-select question to pause and wait for the user to choose the handling method**; must not select automatically. Options:
  ...
  - Option B: After user selects B, run `"$ZCW_BASH" "$ZCW_STATE" transition <change-name> verify-fail`, then invoke `/zcw-build`; `/zcw-build`'s Spec Incremental Update rules will load the Superpowers `brainstorming` skill to update Design Doc + delta spec
...
Verification report must be saved to disk and recorded in `.zcw.yaml`; after branch handling completes, state fields must also be written. Do not manually set `verify_result: pass`; use guard for auto-transition.
...
"$ZCW_BASH" "$ZCW_STATE" set <change-name> verification_report docs/superpowers/reports/YYYY-MM-DD-<change-name>-verify.md
"$ZCW_BASH" "$ZCW_STATE" set <change-name> branch_status handled
```
...
- Branch handled
- `verification_report` in `.zcw.yaml` points to an existing verification report file
- `branch_status: handled` in `.zcw.yaml`
- **Phase guard**: Run `"$ZCW_BASH" "$ZCW_GUARD" <change-name> verify --apply`; after all PASS, auto-transitions to `phase: archive` through `zcw-state transition verify-pass`

After both verification and branch handling are complete, run guard for auto-transition:

```bash
"$ZCW_BASH" "$ZCW_GUARD" <change-name> verify --apply
```
...
Note: after `zcw-archive` starts, it must first execute the final archive confirmation blocking point and wait for the user to explicitly choose "Confirm archive" before running the archive script. Must not automatically archive just because verification passed.

## Context Compression Recovery

Follow `zcw/reference/context-recovery.md` with phase set to `verify`.
