---
name: zcw-open
description: "ZCW Phase 1: Open. Invoke with /zcw-open. Explore ideas through Spec Kit, confirm requirements clarification, then create change structure (proposal + design + tasks)."
---
...
Every prompt and artifact request passed to Spec Kit must include the output-language constraint: use the language of the user request that triggered this workflow. When resuming an existing change with a clear dominant artifact language, preserve that language unless the user explicitly asks to switch.

### 1. Explore Ideas and Clarify Requirements

**Immediately execute:** Use the Skill tool to load the `speckit-explore` skill. Skipping this step is prohibited.
...
When the user input is a large PRD, roadmap, complete product plan, or the clarification summary shows multiple independent capabilities, modules, user journeys, or milestones, must evaluate whether it should be split into multiple changes before creating Spec Kit artifacts.
...
When splitting is recommended, must follow the `zcw/reference/decision-point.md` protocol to pause and wait for the user's choice.

The user choices must include:
- "Create multiple Spec Kit changes" — create independent changes from the proposed split
...
Every accepted split item must be created as an independent change through `/zcw-open`, not by calling `/opsx:new` directly. `/zcw-open` creates both Spec Kit artifacts and `.zcw.yaml`, ensuring each change enters the ZCW state machine.

Must not create spec.md, plan.md, or tasks.md before the user completes the PRD split choice. If the user chooses to create multiple changes, the current `/zcw-open` invocation only completes split confirmation and coordination, then enters `/zcw-open` for each split item in the user-confirmed order.
...
In batch split mode, a single split item must not auto-advance to `/zcw-design` after completing the open phase. After splitting is complete, must pause and ask the user which change to start; after the user chooses, advance only that change into `/zcw-design`, while other changes remain active and can be resumed later through `/zcw`.

Minimal resume rule: do not add a dedicated batch state file. On resume, first check already-created active changes; split items that already exist and contain `.zcw.yaml` must not be created again, while uncreated split items continue through `/zcw-open` according to the user-confirmed split list. If the confirmed split list cannot be recovered from the conversation, must ask the user to confirm the split list again before continuing.

### 1b. Requirements Clarification Completion Confirmation (Blocking Point)

Before creating Spec Kit artifacts, must follow the `zcw/reference/decision-point.md` protocol to pause and wait for the user to confirm requirements clarification is complete.
...
Must not create spec.md, plan.md, or tasks.md before the user confirms requirements clarification is complete, and must not use the Skill tool to load the `speckit-propose` skill to generate all artifacts in one pass.

### 1c. Change Name Confirmation (Blocking Point)

Before creating the change directory (`speckit new change`), must follow the `zcw/reference/decision-point.md` protocol to pause and let the user decide the change name. Must not auto-generate or silently infer the change name.

Spec Kit change names must be **kebab-case English** (lowercase letters, digits, hyphens; e.g. `refine-requirements-doc`). Chinese or other non-conforming names are invalid.
...
Must not run `speckit new change` or create `.zcw.yaml` before the user confirms the final change name. If the chosen/converted name collides with an existing change, report the collision and ask the user to choose another name.
...
Full `/zcw` workflow must not use the Skill tool to load the `speckit-propose` skill by default; only load it when the user explicitly requests generating the proposal and artifacts in one pass.
...
1. Refresh status: `speckit status --change "<name>" --json`
2. Fetch artifact instructions:

   ```bash
   speckit instructions proposal --change "<name>" --json
   speckit instructions design --change "<name>" --json
   speckit instructions tasks --change "<name>" --json
   ```
...
4. After creating each artifact, re-run `speckit status --change "<name>" --json` to confirm status before continuing to the next artifact

**Failure handling**: If `speckit instructions` fails, returns invalid JSON, reports unmet `dependencies`, or does not provide a usable `resolvedOutputPath`, must immediately stop artifact creation and report the Spec Kit error. Must not fall back to hard-coded artifact prose because that would silently bypass project rules.
...
├── .specify
├── .zcw.yaml
├── spec.md       # Why + What: problem, goals, scope
├── plan.md         # How (high-level): architecture decisions, approach selection
└── tasks.md          # Task checklist (checkboxes)
```

Create `.zcw.yaml` state file:
...
**Idempotency**: All open phase operations can be safely re-executed. If `.zcw.yaml` is already at `phase: open` and all three artifact files exist, skip completed steps and continue from the first missing step.

### 4. Content Completeness Check

Confirm the three documents have complete content:
- **spec.md**: problem background, goals, scope, non-goals
- **plan.md**: high-level architecture decisions, approach selection, data flow
...
After the three documents are created and content completeness check passes, **must follow the `zcw/reference/decision-point.md` protocol to pause and wait for user confirmation**. Must not execute phase guard or auto-transition before user confirmation.

The user confirmation question must be presented as a single-select question with the following summary and options:

**Summary content**:
- **spec.md**: problem background, goals, scope
- **plan.md**: high-level architecture decisions, approach selection
...
- spec.md, plan.md, tasks.md all created with complete content
- **User has confirmed** proposal, design, tasks content meets expectations
- **Phase guard**: Run `"$ZCW_BASH" "$ZCW_GUARD" <change-name> open --apply`; after all PASS, auto-transitions to next phase

Must use `--apply` before exit, otherwise `.zcw.yaml` remains at `phase: open` and the next phase entry check will fail.
