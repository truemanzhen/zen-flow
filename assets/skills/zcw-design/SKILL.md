---
name: zcw-design
description: "ZCW Phase 2: Deep Design. Invoke with /zcw-design. Produce Design Doc and delta spec through brainstorming."
---

# ZCW Phase 2: Deep Design (Design)

## Prerequisites

- Active change exists (spec.md, plan.md, tasks.md)
...
### 1a. Generate Spec Kit → Superpowers Handoff Package
...
The script reads the change `.zcw.yaml` `context_compression` snapshot, then generates and records the matching handoff package.
...
Beta mode (`context_compression: beta` in project `.zcw/config.yaml`, snapshotted into `.zcw.yaml` when the change is created) generates:
...
The beta handoff package is a **structured spec projection** that reduces Spec Kit token load without replacing the canonical spec:
...
- Spec Kit delta specs remain canonical; if the projection is missing, stale, or unclear, regenerate the handoff or read the source spec directly instead of writing an agent summary
...
Handoff package sources come from Spec Kit open phase artifacts:
- `spec.md`: goals, motivation, scope, non-goals
- `plan.md`: high-level architecture decisions, approach constraints
...
Spec Kit artifacts are the upstream source of truth, but you must not weaken the Superpowers `brainstorming` clarification flow by "skipping redundant context exploration".
...
The brainstorming phase does not write to the Design Doc file; it only produces design proposals for Step 1c user confirmation. Only after confirmation should `docs/superpowers/specs/YYYY-MM-DD-<topic>-plan.md` be created and delta spec written back.
...
After brainstorming produces a design proposal, **must follow the `zcw/reference/decision-point.md` protocol to pause and wait for the user to explicitly confirm the design proposal**. Must not create the final Design Doc, write `design_doc`, run design guard, or enter `/zcw-build` before user confirmation.
...
`specs/<name>/.zcw/handoff/brainstorm-summary.md` structure:
...
- `specs/<name>/.zcw/handoff/design-context.md` (or `spec-context.md` in beta mode)
- `specs/<name>/.zcw/handoff/design-context.json` (or `spec-context.json` in beta mode)

### 1e. Active Context Compaction Gate

After Step 1d completes and `brainstorm-summary.md` is written, enter the active compaction gate before creating the Design Doc. At this point the Spec Kit handoff, brainstorming decisions, and pending items are durable, so the agent should proactively release the earlier Spec and brainstorming context to preserve window space for Step 2 and the later Build phase.
...
Write the Design Doc to `docs/superpowers/specs/YYYY-MM-DD-<topic>-plan.md`.
...
- Design Doc frontmatter contains `zcw_change`, `role: technical-design`, `canonical_spec: speckit`
- `handoff_context` and `handoff_hash` written to `.zcw.yaml` (enforced by guard)
- `handoff_hash` matches current Spec Kit open phase artifacts (enforced by guard)
...
- If new capabilities or supplementary acceptance scenarios exist, Spec Kit delta spec has been created/updated
- `design_doc` written to `.zcw.yaml`
- **Phase guard**: Run `"$ZCW_BASH" "$ZCW_GUARD" <change-name> design --apply`; after all PASS, auto-transitions to `phase: build`
...
Follow `zcw/reference/context-recovery.md` with phase set to `design`.
