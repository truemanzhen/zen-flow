---
name: zcw-build
description: "ZCW 阶段 3：计划与构建。用 /zcw-build 调用。制定计划并选择执行方式（subagent 或直接执行）实施。"
---

# ZCW 阶段 3：计划与构建（Build）
...
. "$ZCW_ENV"
"$ZCW_BASH" "$ZCW_STATE" check <name> build
```

验证通过后继续 Step 1。验证失败时脚本会输出具体失败原因。

**幂等性**：build 阶段所有操作可安全重复执行。读取 `.zcw.yaml` 的 `phase` 字段确认仍在 build 阶段，读取 plan 文件头的 `base-ref`，再用 `grep -n '\- \[ \]' tasks.md | head -1` 找到第一个未勾选任务继续执行。已提交的任务不得重复提交。
...
```yaml
---
change: <speckit-change-name>
design-doc: docs/superpowers/specs/YYYY-MM-DD-<topic>-plan.md
...
```bash
"$ZCW_BASH" "$ZCW_STATE" set <name> plan docs/superpowers/plans/YYYY-MM-DD-feature.md
```
...
| B | 暂停切换模型 | 记录 `build_pause: plan-ready`，本次 `/zcw-build` 停止，用户稍后可从 `/zcw` 或 `/zcw-build` 恢复 |

这是用户决策点。**必须按 `zcw/reference/decision-point.md` 的协议暂停并等待用户明确选择**，不得自动继续，也不得把暂停写入 `build_mode`。

用户选择继续时：

```bash
"$ZCW_BASH" "$ZCW_STATE" set <name> build_pause null
```

用户选择暂停时：

```bash
"$ZCW_BASH" "$ZCW_STATE" set <name> build_pause plan-ready
```
...
这是用户决策点。**必须按 `zcw/reference/decision-point.md` 的协议暂停并等待用户明确选择隔离方式、执行方式、TDD 模式和代码审查模式**，不得根据推荐规则自行选择 `branch` 或 `worktree`，也不得根据推荐规则自行选择执行方式、TDD 模式或代码审查模式。推荐规则只能用于说明建议，不能替代用户确认。
...
- 若用户选择 `executing-plans`：运行 `"$ZCW_BASH" "$ZCW_STATE" set <name> subagent_dispatch null`，再运行 `"$ZCW_BASH" "$ZCW_STATE" set <name> build_mode executing-plans`
- 若用户选择 `subagent-driven-development`：先确认当前平台存在可调用的真实后台 subagent / Task / multi-agent 调度能力；确认后先运行 `"$ZCW_BASH" "$ZCW_STATE" set <name> subagent_dispatch confirmed`，再运行 `"$ZCW_BASH" "$ZCW_STATE" set <name> build_mode subagent-driven-development`
...
运行 `"$ZCW_BASH" "$ZCW_STATE" set <name> tdd_mode <tdd|direct>`
...
运行 `"$ZCW_BASH" "$ZCW_STATE" set <name> review_mode <off|standard|thorough>`

`isolation` 是脚本级硬约束。full workflow 初始化时可以为 `null`，但只允许存在到本步骤之前。若保持 `null`，`build → verify` 的 guard 和 `zcw-state transition build-complete` 都会失败。

`subagent_dispatch` 是脚本级硬约束。`build_mode: subagent-driven-development` 离开 build 阶段前必须同时满足 `subagent_dispatch: confirmed`，否则 `zcw-guard.sh build --apply` 和 `zcw-state transition build-complete` 都会失败。

`tdd_mode` 是脚本级硬约束。full workflow 离开 build 阶段前 `tdd_mode` 必须已选择为 `tdd` 或 `direct`，否则 `zcw-guard.sh build --apply` 和 `zcw-state transition build-complete` 都会失败。

`review_mode` 是脚本级硬约束。新建 full workflow 离开 build 阶段前 `review_mode` 必须已选择为 `off`、`standard` 或 `thorough`，否则 `zcw-guard.sh build --apply` 和 `zcw-state transition build-complete` 都会失败。旧状态文件若没有该字段，按兼容路径继续，但恢复时应补写该字段。

`build_mode` 默认仅 hotfix/tweak preset 使用 `direct`。full workflow 不得默认使用 `direct`。只有用户明确要求跳过计划执行技能，且你已记录显式 override 时，才允许：

```bash
"$ZCW_BASH" "$ZCW_STATE" set <name> direct_override true
"$ZCW_BASH" "$ZCW_STATE" set <name> build_mode direct
```
...
- `build_mode: subagent-driven-development`：主会话只负责协调，禁止直接编写实现代码。**立即执行：** 使用 Skill 工具加载 Superpowers `subagent-driven-development` 技能。技能加载后，读取 `zcw/reference/subagent-dispatch.md` 获取 ZCW 专属扩展（真实后台调度、任务隔离、勾选验证、TDD 约束、连续执行、上下文恢复），与技能工作流配合应用。若两者发生冲突，以更具体的 ZCW 扩展为准。
- 如果当前平台没有真实后台 agent 调度能力，必须暂停并等待用户选择改用主窗口执行。用户选择改用主窗口执行后，必须先运行 `"$ZCW_BASH" "$ZCW_STATE" set <name> build_mode executing-plans`，再按 `build_mode: executing-plans` 分支加载 Superpowers `executing-plans` 技能。用户未明确选择前，不得继续执行任务。
...
- `build_mode: subagent-driven-development`：主会话不加载 TDD skill；TDD 约束和证据门槛已在 `zcw/reference/subagent-dispatch.md` 中定义，每个后台 implementer 和修复 agent 必须自行使用 Skill 工具加载 Superpowers `test-driven-development` 技能，并遵循 ZCW 注入的 TDD 硬约束。
...
要求：
- `requesting-code-review` 技能必须在 `"$ZCW_BASH" "$ZCW_GUARD" <change-name> build --apply` 之前加载
...
具体调查、最小失败测试、修复验证和保持当前 change 验证闭环的要求，按 `zcw/reference/debug-gate.md` 执行。
...
| 规模 | 触发条件 | 做法 |
|------|---------|------|
| 小 | 遗漏验收场景、边界条件 | 直接编辑 delta spec + plan.md，追加 tasks.md 任务 |
| 中 | 接口变更、新增组件、数据流变化 | **使用当前平台可用的用户输入/确认机制暂停并等待用户确认后**，必须使用 Skill 工具加载 Superpowers `brainstorming` 更新 Design Doc + delta spec |
| 大 | 全新 capability 需求 | **必须使用当前平台可用的用户输入/确认机制暂停并等待用户确认拆分**；用户确认后，通过 `/zcw-open` 创建独立 change |

**50% 阈值判定**：以 tasks.md 初始任务总数为基准，若新增任务数超过该总数的一半，视为超出原计划范围，**必须按 `zcw/reference/decision-point.md` 的协议暂停并等待用户决定是否拆分为新 change**。

创建独立 change 时必须调用 `/zcw-open`，不得直接调用 `/opsx:new`。`/zcw-open` 会同时创建 Spec Kit 产物和 `.zcw.yaml`，避免新 change 脱离 ZCW 状态机。

**用户选择必须包含**：
- 「拆分为新 change」— 通过 `/zcw-open` 创建独立 change
...
- **上下文压缩后恢复**：按 `zcw/reference/context-recovery.md` 执行，phase 参数为 `build`。
- **用户手动修改恢复**：按 `zcw/reference/dirty-worktree.md` 协议处理未提交改动。该协议定义了检查步骤、归因分类和禁令。build 阶段的特殊处理：
...
- **阶段守卫**：运行 `"$ZCW_BASH" "$ZCW_GUARD" <change-name> build --apply`，全部 PASS 后由守卫推进到 `phase: verify`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）
...
配置位置可为 change 的 `.zcw.yaml`，也可为仓库根目录的 `.zcw.yaml` / `zcw.yaml` / `.zcw.yml` / `zcw.yml`。
...
按 `zcw/reference/auto-transition.md` 执行。关键命令：
