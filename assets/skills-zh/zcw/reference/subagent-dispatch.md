# Subagent 驱动开发的 ZCW 扩展

规范路径：`zcw/reference/subagent-dispatch.md`

本文档提供在 Superpowers `subagent-driven-development` 技能**之上**应用的 ZCW 专属扩展。该技能负责核心派发循环（每个 task 派发全新 implementer → spec compliance review → code quality review → 下一个 task）并强制连续执行。本文档添加 ZCW 特有的真实后台调度、任务追踪、状态验证、代码审查模式和上下文恢复。若 Superpowers 技能与本文档发生冲突时，以本文档中更具体的 ZCW 约束为准。
...
2. 为每个 task 保存唯一标识：plan 中 checkbox 后的完整任务文本，以及它映射的 Spec Kit task 完整文本（若存在）。若文本不唯一，停止并先修正计划，禁止依赖"第一个匹配项"。
...
主会话**仅负责协调**，禁止直接执行 task。主会话禁止修改源代码。协调者唯一允许的文件修改是 plan、Spec Kit task 和 subagent 进度检查点的持久化更新。不得把多个 task 打包给同一个 agent。每个 task 派发一个全新的后台 implementer agent；当 `review_mode` 需要审查或修复时，spec reviewer、code quality reviewer、修复 agent 和 final reviewer 也必须分别使用全新的后台 agent：
...
implementer 只负责实现、测试和提交代码。**implementer 不得勾选 plan 或 Spec Kit task**，也不得只更新内置 Todo 或对话 checklist。
...
主会话必须维护 `specs/<name>/.zcw/subagent-progress.md`，并在每次派发、agent 回报、审查结果、修复轮次变化和 task 勾选后立即更新。检查点至少记录：

- 当前 plan task 唯一文本及映射的 Spec Kit task 文本
...
该文件只保存恢复所需的协调状态，不替代 plan 或 Spec Kit checkbox。当前 task 完成后保留其最终记录，开始下一个 task 时用下一 task 的记录替换。
...
- 所有 task 完成后，若 `review_mode: standard`，将检查点切换为 `final-review`，只派发一次最终轻量 code reviewer。CRITICAL 或 IMPORTANT 问题最多自动修复和复查一轮；仍未通过则暂停交给用户。通过或接受非 CRITICAL 发现后继续返回 `zcw-build`。
- 所有 task 完成后，若 `review_mode: thorough`，将检查点切换为 `final-review`，派发一次最终完整 reviewer。CRITICAL 或 IMPORTANT 问题最多自动修复和复查两轮；仍未通过则暂停交给用户。通过或接受非 CRITICAL 发现后继续返回 `zcw-build`。
- 所有 task 完成后，若 `review_mode: off`，不进入 `final-review` 或 `final-fix`，但必须在持久产物中记录跳过自动代码审查的原因，然后返回 `zcw-build`。
- final review 通过后，结束的只是 subagent 派发循环，不是 ZCW workflow。不得加载 `finishing-a-development-branch`，不得停下来询问用户下一步；必须返回 `zcw-build` 继续执行退出条件、阶段守卫和后续阶段衔接。

## 上下文恢复

重新加载 Superpowers `subagent-driven-development` 技能并重新阅读本文档。先读取 `specs/<name>/.zcw/subagent-progress.md`，再与第一个未勾选 task 和当前工作树核对：
