...
- **如何做跨平台 Skill 分发和安装** — Comet 支持多种 AI 编码平台、项目级/全局安装、仅英文版 ZCW Skill 资产，以及平台差异化目录（例如
...
| `--language <lang>` | CLI 提示/输出语言：`en` 或 `zh`（ZCW skill 始终安装英文版） |
...
<details>
<summary><code>zcw-mcp</code> — 启动只读 ZCW MCP 服务</summary>

启动基于 stdio 的 MCP 服务，供支持 Model Context Protocol 的 AI 客户端调用。
该服务只暴露工作流检查工具，不写文件，也不会推进阶段。

| 工具                       | 描述                         |
|--------------------------|----------------------------|
| `zcw_status`             | 当前项目摘要和下一步建议              |
| `zcw_dashboard_snapshot` | 仪表盘快照 JSON                 |
| `zcw_change_detail`      | 单个 change 的状态、产物和 handoff 信息 |
| `zcw_doctor`             | 机器可读的 doctor 检查结果          |

可以通过 `ZCW_ENABLED_TOOLS=zcw_status,zcw_doctor` 限制暴露的工具。默认值为 `all`。
...
| `--language <lang>` | 覆盖 CLI 提示/输出语言 (`en`, `zh`)；刷新后的 ZCW skill 始终安装英文版 |
