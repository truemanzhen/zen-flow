...
setup() {
  ...
  export SCRIPT_SOURCE="$BATS_TEST_DIRNAME/../../assets/skills/zcw/scripts/zcw-state.sh"
  export SCRIPT_PATH="$TEST_TMPDIR/zcw-state.sh"
  cd "$TEST_TMPDIR"
...
@test "init creates .zcw.yaml with full workflow defaults" {
  run bash "$SCRIPT_PATH" init my-change full
  [ "$status" -eq 0 ]
  [ -f "openspec/changes/my-change/.zcw.yaml" ]
  grep -q "phase: open" "openspec/changes/my-change/.zcw.yaml"
  grep -q "verify_mode: null" "openspec/changes/my-change/.zcw.yaml"
  grep -q "verification_report: null" "openspec/changes/my-change/.zcw.yaml"
  grep -q "branch_status: pending" "openspec/changes/my-change/.zcw.yaml"
}

@test "init creates .zcw.yaml with hotfix workflow defaults" {
  run bash "$SCRIPT_PATH" init hotfix-123 hotfix
  [ "$status" -eq 0 ]
  grep -q "phase: open" "openspec/changes/hotfix-123/.zcw.yaml"
  grep -q "build_mode: direct" "openspec/changes/hotfix-123/.zcw.yaml"
}

@test "init creates .zcw.yaml with tweak workflow defaults" {
  run bash "$SCRIPT_PATH" init tweak-abc tweak
  [ "$status" -eq 0 ]
  grep -q "phase: open" "openspec/changes/tweak-abc/.zcw.yaml"
  grep -q "isolation: branch" "openspec/changes/tweak-abc/.zcw.yaml"
}

@test "init rejects duplicate .zcw.yaml" {
...
@test "check open fails without .zcw.yaml" {
...
@test "check open fails if phase is not open" {
  bash "$SCRIPT_PATH" init my-change full
  ZCW_FORCE_PHASE=1 bash "$SCRIPT_PATH" set my-change phase design
  run bash "$SCRIPT_PATH" check my-change open
  [ "$status" -ne 0 ]
}
