...
const scriptsDir = path.resolve('assets', 'skills', 'zcw', 'scripts');
...
function runBash(
  ...
  if (!bashCommand) {
    throw new Error('zcw shell script tests require Bash or Git Bash');
  }
...
describe('zcw shell script contracts', () => {
  ...
  it('keeps zcw-hook-guard blocked messages in English', async () => {
    const hookSource = await fs.readFile(path.join(scriptsDir, 'zcw-hook-guard.sh'), 'utf-8');
...
describeShell('zcw shell scripts', () => {
  let tmpDir: string;
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `zcw-scripts-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    ...
    guardScript = path.join(tmpScriptsDir, 'zcw-guard.sh');
    stateScript = path.join(tmpScriptsDir, 'zcw-state.sh');
    hookGuardScript = path.join(tmpScriptsDir, 'zcw-hook-guard.sh');
  });
  ...
  it('initializes a new change directory with workflow defaults', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'new-full-change', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('initializes build_pause as null for new changes', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'pause-defaults', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('initializes subagent_dispatch as null for new changes', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'subagent-dispatch-defaults', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('initializes tdd_mode as null for full workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'tdd-defaults', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('initializes review_mode as null for full workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'review-defaults', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('initializes review_mode as off for hotfix workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'review-hotfix', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('initializes tdd_mode as direct for hotfix workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'tdd-hotfix', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('initializes context_compression as off by default', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-defaults', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('snapshots beta context compression from .zcw/config.yaml when initializing a change', async () => {
    await writeFile(path.join(tmpDir, '.zcw', 'config.yaml'), 'context_compression: beta\n');

    const result = runBash(tmpDir, stateScript, ['init', 'context-beta', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-beta', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('snapshots review_mode from .zcw/config.yaml when initializing a full change', async () => {
    await writeFile(path.join(tmpDir, '.zcw', 'config.yaml'), 'review_mode: standard\n');

    const result = runBash(tmpDir, stateScript, ['init', 'review-standard', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'review-standard', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('rejects invalid review_mode from .zcw/config.yaml when initializing a change', async () => {
    await writeFile(path.join(tmpDir, '.zcw', 'config.yaml'), 'review_mode: noisy\n');

    const result = runBash(tmpDir, stateScript, ['init', 'review-invalid', 'full']);

    expect(result.status).toBe(1);
    expect(result.stderr).toContain("Invalid review_mode from .zcw/config.yaml: 'noisy'");
    expect(result.stderr).toContain('Valid values: off, standard, thorough');
  }, 20_000);

  it('lets ZCW_CONTEXT_COMPRESSION override the project context compression default', async () => {
    await writeFile(path.join(tmpDir, '.zcw', 'config.yaml'), 'context_compression: beta\n');

    const result = runBash(tmpDir, stateScript, ['init', 'context-env', 'full'], {
      ZCW_CONTEXT_COMPRESSION: 'off',
    });
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-env', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('initializes auto_transition as true when openspec zcw config is absent', async () => {
    const result = runBash(tmpDir, stateScript, ['init', 'auto-transition-defaults', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'auto-transition-defaults', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('initializes auto_transition from .zcw/config.yaml when set to false', async () => {
    await fs.mkdir(path.join(tmpDir, '.zcw'), { recursive: true });
    await writeFile(
      path.join(tmpDir, '.zcw', 'config.yaml'),
      'context_compression: off\nauto_transition: false\n',
    );

    const result = runBash(tmpDir, stateScript, ['init', 'auto-transition-config-false', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'auto-transition-config-false', '.zcw.yaml'),
      'utf-8',
    );
    const get = runBash(tmpDir, stateScript, [
      'get',
  ...
  it('next resolves auto for full workflow when auto_transition is true', async () => {
    ...
    expect(result.stdout).toContain('SKILL: zcw-verify');
  }, 20_000);
  ...
  it('next resolves manual with hint when auto_transition is false', async () => {
    ...
    expect(result.stdout).toContain('SKILL: zcw-build');
  ...
  it('next maps hotfix and tweak workflows to their preset skills in build phase', async () => {
    ...
    expect(hotfix.stdout).toContain('SKILL: zcw-hotfix');
    expect(tweak.stdout).toContain('SKILL: zcw-tweak');
  }, 20_000);
  ...
  it('next maps each non-build phase to the owning skill', async () => {
    ...
    expect(design.stdout).toContain('SKILL: zcw-design');
    expect(archive.stdout).toContain('SKILL: zcw-archive');
  }, 20_000);

  it('next exits non-zero when .zcw.yaml is missing', async () => {
    const result = runBash(tmpDir, stateScript, ['next', 'next-missing']);

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('.zcw.yaml not found');
  }, 20_000);
  ...
  it('zcw-env.sh exports bundled script paths from its own directory', async () => {
    const envScript = path.join(tmpDir, 'scripts', 'zcw-env.sh');
    ...
    await writeFile(
      ...
      [
        ...
        'printf "%s\\n%s\\n%s\\n%s\\n%s\\n" "$ZCW_STATE" "$ZCW_GUARD" "$ZCW_HANDOFF" "$ZCW_ARCHIVE" "$ZCW_BASH"',
        '',
      ].join('\n'),
    );
    ...
    expect(result.stdout).toContain('zcw-state.sh');
    expect(result.stdout).toContain('zcw-guard.sh');
    expect(result.stdout).toContain('zcw-handoff.sh');
    expect(result.stdout).toContain('zcw-archive.sh');
    expect(result.stdout).toContain('bash');
  }, 20_000);

  it('zcw-env.sh returns failure when a bundled script is missing', async () => {
    const envScript = path.join(tmpDir, 'scripts', 'zcw-env.sh');
    await fs.rm(path.join(tmpDir, 'scripts', 'zcw-guard.sh'));
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain('ERROR: ZCW scripts not found');
    expect(result.stdout).toContain('source-status=1');
  }, 20_000);

  it('zcw-env.sh does not change caller shell options when sourced', async () => {
    const envScript = path.join(tmpDir, 'scripts', 'zcw-env.sh');
  ...
  it('generates a design handoff and requires minimal design doc linkage before leaving design', async () => {
    ...
    expect(contextPath).toBe('openspec/changes/handoff-change/.zcw/handoff/design-context.json');
  ...
  it('generates a beta spec projection handoff with verbatim spec content', async () => {
    ...
    expect(handoff.status).toBe(0);
    expect(contextPath).toBe('openspec/changes/beta-context/.zcw/handoff/spec-context.json');
    ...
    expect(contextMarkdown).toContain('Generated-by: zcw-handoff.sh');
  ...
  it('reads zcw yaml fields without including trailing comments', async () => {
    const handoffScript = path.join(tmpDir, 'scripts', 'zcw-handoff.sh');
    const validateScript = path.join(tmpDir, 'scripts', 'zcw-yaml-validate.sh');
    await createChange(
      tmpDir,
      'commented-yaml',
      [
  ...
  it('warns when --full is passed in beta mode', async () => {
    ...
    expect(contextPath).toBe('openspec/changes/beta-full-warn/.zcw/handoff/spec-context.json');
  }, 20_000);
  ...
  it('blocks design exit when design doc frontmatter is missing required fields', async () => {
    ...
    // Design doc with wrong zcw_change
    await writeFile(
      path.join(tmpDir, 'docs', 'superpowers', 'specs', 'bad-design.md'),
      [
        '---',
        'zcw_change: wrong-change',
        'role: technical-design',
        'canonical_spec: openspec',
        '---',
        '',
  ...
  it('rejects invalid build_pause values during schema validation', async () => {
    ...
    expect(result.stderr).toContain('FATAL: .zcw.yaml schema validation failed');
  }, 20_000);
  ...
  it('keeps optional YAML field reads safe under pipefail', async () => {
    for (const name of ['zcw-state.sh', 'zcw-guard.sh']) {
      const content = await fs.readFile(path.join(tmpDir, 'scripts', name), 'utf-8');
  ...
  it('guards bash uname detection when bash cannot be spawned', async () => {
    const files = [
      ...
      path.resolve('test', 'ts', 'zcw-scripts.test.ts'),
    ];
  ...
  it('uses ZCW_BASH for nested script calls when PATH bash is unusable', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'nested-bash', '.zcw.yaml'),
      'utf-8',
    );
  ...
  it('does not use PATH bash for nested ZCW script calls', async () => {
    for (const name of ['zcw-archive.sh', 'zcw-guard.sh', 'zcw-handoff.sh']) {
      const content = await fs.readFile(path.join(tmpDir, 'scripts', name), 'utf-8');

      expect(content, `${name} should use ZCW_BASH for nested scripts`).not.toMatch(
  ...
  it('uses root-level build command config before inferred build commands', async () => {
    ...
    await writeFile(path.join(tmpDir, 'zcw.yaml'), 'build_command: node root-build-check.js\n');
    await writeFile(
  ...
  it('reports accurate archive step counts when syncing and annotating', async () => {
    const archiveScript = path.join(tmpDir, 'scripts', 'zcw-archive.sh');
    ...
    const result = runBash(tmpDir, archiveScript, ['ready-to-archive'], {
      ZCW_OPENSPEC: toBashPath(fakeOpenSpec),
    });
  ...
  it('blocks direct phase writes but allows the ZCW_FORCE_PHASE escape hatch', async () => {
    await createChange(
      tmpDir,
      'phase-jump',
      ['workflow: full', 'phase: open', 'design_doc: null', 'archived: false', ''].join('\n'),
  ...
  describe('check --recover', () => {
    ...
    it('outputs recovery context for design phase with handoff but no design doc', async () => {
      await createChange(
        ...
        [
          ...
          'plan: null',
          'verify_result: pending',
          'handoff_context: openspec/changes/recover-design/.zcw/handoff/design-context.json',
    ...
    it('outputs recovery context for archive phase', async () => {
      ...
      expect(result.stdout).toContain('/zcw-archive');
      expect(result.stdout).toContain('End Recovery Context');
    });
  ...
  describe('review fix: build-complete conditional reset', () => {
    it('preserves verification_report on re-verify cycle (H1)', async () => {
      ...
      expect(result.status).toBe(0);
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'reverify-test', '.zcw.yaml'),
        'utf-8',
      );
  ...
  describe('review fix: verify-fail preserves branch_status', () => {
    it('does not reset branch_status on verify-fail (H6)', async () => {
      ...
      expect(result.status).toBe(0);
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'branch-preserve', '.zcw.yaml'),
        'utf-8',
      );
  ...
  describe('review fix: command injection prevention', () => {
    it('rejects build_command with shell metacharacters (C3)', async () => {
      ...
      // No ZCW_SKIP_BUILD — run_command_string should reject before executing
  ...
  describe('zcw-hook-guard.sh — phase write guard', () => {
    it('allows all writes when no active zcw change exists', async () => {
    ...
    it('allows new change artifact writes while another change is in archive phase (no state file yet)', async () => {
      ...
      // Brand-new change being created: artifacts written before .zcw.yaml exists
